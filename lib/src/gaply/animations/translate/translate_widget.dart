...
class GaplyTranslateWidget extends StatefulWidget {
  final Widget child;
  final GaplyTranslate style;

  const GaplyTranslateWidget({super.key, required this.child, required this.style});

  @override
  State<GaplyTranslateWidget> createState() => GaplyTranslateWidgetState();
}

class GaplyTranslateWidgetState extends State<GaplyTranslateWidget> with SingleTickerProviderStateMixin {
  ...
  @override
  void didUpdateWidget(covariant GaplyTranslateWidget oldWidget) {
  ...
  void _execute(GaplyTranslate style) {
    if (!mounted) return;
  ...
  void executeParams(GaplyTranslate style) {
