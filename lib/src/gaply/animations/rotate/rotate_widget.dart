...
class GaplyRotateWidget extends StatefulWidget {
  final Widget child;
  final GaplyRotate style;

  const GaplyRotateWidget({super.key, required this.child, required this.style});

  @override
  State<GaplyRotateWidget> createState() => GaplyRotateWidgetState();
}

class GaplyRotateWidgetState extends State<GaplyRotateWidget> with SingleTickerProviderStateMixin {
  ...
  @override
  void didUpdateWidget(GaplyRotateWidget oldWidget) {
  ...
  void _execute(GaplyRotate style) {
    if (!mounted) return;
  ...
  void executeParams(GaplyRotate style) {
